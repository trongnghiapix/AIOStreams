...
For multi-value attributes (`visualTag`, `audioTag`, `audioChannel`, `language`) a stream is assigned to the **first** group key that matches the `filterValues` list, or to its first value when no filter is specified. A stream is never included more than once in the output.
